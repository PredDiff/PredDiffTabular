...
def get_model_and_data(max_epochs=1, retrain=True):
    ...
    if retrain is True:
        ...
        model.T = float(model_tsc.temperature.detach().numpy())
...
def get_relevances(explainer, data: pd.DataFrame, img_params):
    # calculate everything
    list_superpixels = generate_list_superpixel(iparam=img_params)
    m_list = explainer.relevances(df_test=data, impute_cols=list_superpixels, n_imputations=100)
...
def get_interaction(explainer, data: pd.DataFrame, iparam, m_list, i_reference, key='mean'):
    ...
    m_interaction = explainer.interactions(df_test=data, interaction_cols=list_interaction[:],
